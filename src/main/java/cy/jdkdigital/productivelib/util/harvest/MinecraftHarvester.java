...
public class MinecraftHarvester
{
    ...
    public static void harvestBlock(Level level, BlockPos pos) {
        ...
        if (cropBlock instanceof AttachedStemBlock stemBlock) {
            ...
            FakePlayer fakePlayer = FakePlayerFactory.get((ServerLevel) level, new GameProfile(HarvestCompatHandler.FARMER_UUID, "productive_farmer"));
            fakePlayer.gameMode.useItemOn(fakePlayer, level, ItemStack.EMPTY, InteractionHand.MAIN_HAND, new BlockHitResult(Vec3.ZERO, Direction.DOWN, pos, true));
